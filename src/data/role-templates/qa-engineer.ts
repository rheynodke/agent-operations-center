...
description: "WAJIB DIGUNAKAN: Ketika diminta membuat regression test plan, evaluasi impact dari perubahan, atau prepare untuk release testing."
---

# Regression Test Planner

Plan dan execute regression tests untuk memastikan perubahan baru tidak break existing functionality.

<HARD-GATE>
Regression test WAJIB dijalankan sebelum setiap release.
Semua critical user journeys HARUS ada dalam regression suite.
Jangan approve release jika ada regression failure.
</HARD-GATE>
...
1. **Impact Analysis** — Identifikasi modul yang terpengaruh oleh perubahan
2. **Test Case Selection** — Pilih test cases yang relevan dari existing suite
3. **New Test Cases** — Buat test cases baru untuk perubahan spesifik
4. **Environment Setup** — Pastikan test environment siap
5. **Execute Regression** — Run semua selected test cases
6. **Document Results** — Pass/Fail per test case dengan evidence
7. **[HUMAN GATE — QA Lead/PM]** — Kirim regression report via notify.sh
8. **Output Document** — Write to outputs/YYYY-MM-DD-regression-report-{feature}.md

## Process Flow

\`\`\`dot
digraph regression_planner {
  rankdir=TB
  node [shape=box, style=rounded]
  impact [label="Impact Analysis"]
  select [label="Test Case Selection"]
  newcases [label="New Test Cases"]
  env [label="Environment Setup"]
  execute [label="Execute Regression"]
  results [label="Document Results"]
  gate [label="HUMAN GATE\\nQA Lead/PM", shape=diamond, style="filled", fillcolor="#f59e0b"]
  approve [label="Release Approved"]
  block [label="Release Blocked\\nBug Fix Required"]
  output [label="Write Regression Report"]

  impact -> select -> newcases -> env -> execute -> results -> gate
  gate -> approve [label="All Pass"]
  gate -> block [label="Failures"]
  block -> execute [label="After Fix"]
  approve -> output
}
\`\`\`
...
**Input dari:** SWE (Agent 4) — PR, code changes, unit test results
**Output ke:** Doc Writer (Agent 6) — QA sign-off for release notes

## Anti-Pattern

- Jangan skip regression karena "perubahan kecil"
- Jangan approve jika ada unsolved P1/P2 bugs
- Jangan document "mostly works" — be precise about failures
`,

    'pipeline-gate': `---
name: pipeline-gate
description: "WAJIB DIGUNAKAN: Sebelum approve release — final quality gate check untuk coverage, test results, dan open bugs."
---

# Pipeline Gate

Final quality gate sebelum release. Checks: coverage threshold, test results, open bug count, regression status.

<HARD-GATE>
Coverage HARUS >= 80% — tidak ada exception.
Tidak ada open P1 (Critical) bugs saat release.
Regression tests HARUS selesai dan pass.
Release tanpa pipeline gate = release tidak diizinkan.
</HARD-GATE>
...
1. **Coverage Check** — \`./scripts/coverage-check.sh\` — must be >= 80%
2. **Test Results Review** — Semua unit + integration tests pass?
3. **Open Bug Audit** — Check GitHub issues — ada P1/P2 open bugs?
4. **Regression Status** — Regression test suite: all pass?
5. **Performance Baseline** — Ada regression performa yang signifikan?
6. **Generate Gate Report** — PASS/FAIL per criterion
7. **[HUMAN GATE — PM/CTO]** — Send gate report via notify.sh
8. **Output Document** — Write to outputs/YYYY-MM-DD-pipeline-gate-{version}.md

## Process Flow

\`\`\`dot
digraph pipeline_gate {
  rankdir=TB
  node [shape=box, style=rounded]
  cov [label="Coverage\\n>= 80%?", shape=diamond]
  tests [label="All Tests Pass?", shape=diamond]
  bugs [label="No P1 Bugs?", shape=diamond]
  regression [label="Regression Pass?", shape=diamond]
  gate [label="HUMAN GATE\\nPM/CTO Final", shape=diamond, style="filled", fillcolor="#f59e0b"]
  release [label="RELEASE APPROVED"]
  block [label="RELEASE BLOCKED"]

  cov -> tests [label="Yes"]
  tests -> bugs [label="Yes"]
  bugs -> regression [label="Yes"]
  regression -> gate [label="Yes"]
  gate -> release [label="Approved"]
  cov -> block [label="No"]
  tests -> block [label="No"]
  bugs -> block [label="No"]
  regression -> block [label="No"]
  gate -> block [label="Rejected"]
}
\`\`\`

## Inter-Agent Handoff

**Input dari:** SWE (Agent 4) — coverage report, test results; Regression Planner — regression report
**Output ke:** Doc Writer (Agent 6) — release approval for release notes

## Anti-Pattern

- Jangan bypass gate karena deadline pressure
- Jangan mark P1 bugs sebagai P2 untuk bypass gate
- Jangan approve partial coverage — 79.9% bukan 80%
`,
  },
